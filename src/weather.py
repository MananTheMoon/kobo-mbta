...
class yawkWeather:
    ...
    def get_weather_forecast(self) -> List[weather_forecast]:
        ...
        for d in range(5):
            ...
            for forecast in forecasts:
                ...
                if max < max1:
                    max = max1
                day_condition.append(
                    str(forecast.getElementsByTagName("symbol")[0].getAttribute("name"))
                )
                icon.append(
                    str(forecast.getElementsByTagName("symbol")[0].getAttribute("var"))
                )
    ...
    def get_weather_current(self) -> weather_current:
        ...
        current_temperature_float = float(
            dom.getElementsByTagName("temperature")[0].getAttribute("value")
        )
        current_humidity = float(
            dom.getElementsByTagName("humidity")[0].getAttribute("value")
        )
        current_condition = dom.getElementsByTagName("weather")[0].getAttribute("value")
        current_icon = (
            "icons/"
            + dom.getElementsByTagName("weather")[0].getAttribute("icon")
            + ".png"
        )
        current_wind = (
            float(dom.getElementsByTagName("speed")[0].getAttribute("value")) * 3.6
        )
